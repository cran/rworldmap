...
%9/2/2011
%to stop R producing Rplots.pdf
<<echo=FALSE>>=
require(grDevices)
...
for(i in 1:10)
   {
    ...
    mapParams <- mapCountryData(sPDF,nameColumnToPlot="CLIMATE", addLegend=FALSE, colourPalette=colourPalette, mapTitle=brewerList[i] )
    do.call(addMapLegend, c(mapParams,horizontal=FALSE,legendLabels="none",legendWidth=0.7))
   }
par(op)   
@
